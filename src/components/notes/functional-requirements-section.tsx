...
export function FunctionalRequirementsSection() {
  ...
  const items = useNotesStore((s) => s.notes.functionalRequirements)
  const { addFunctionalReq, toggleFunctionalReq, removeFunctionalReq, updateFunctionalReq } =
    useNotesStore()

  const handleAdd = () => {
    addFunctionalReq('')
    const latest = useNotesStore.getState().notes.functionalRequirements
        ...
        <div className="px-4 pb-3">
          {items.length === 0 && (
            <p className="text-xs py-2" style={{ color: 'var(--color-text-muted)' }}>
              What should the system do? Add functional requirements to track.
            </p>
          )}
          {items.map((item) => (
            <ChecklistItem
